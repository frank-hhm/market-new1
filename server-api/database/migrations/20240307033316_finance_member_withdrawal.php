...
class FinanceMemberWithdrawal extends Migrator
{
    public function change()
    {
        $table = $this->table('finance_member_withdrawal');
        $table
            ->addColumn(Column::integer('member_id')->setLimit(11)->setDefault(0)->setComment('代理ID'))
            ...
            ->setPrimaryKey('id')
            ->addIndex(['id','member_id','agent_id','status'])
            ->setComment('用户提现申请')
            ->create();
    }
}
