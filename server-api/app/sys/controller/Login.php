...
class Login extends BaseController
{
    ...
    public function index()
    {
        ...
        try {
            ...
            $login = AdminService::instance()->login($account, $data['password'], 'admin');
            throw new CommonException('',0);
        }catch (\Exception $e){
            $loginLogService = app(OperateLogsService::class);
            $adminId = $login['user_info']['id'] ?? 0;
            if(empty($adminId) && !empty($account)){
                $adminId = AdminService::instance()->accountByAdminToId($account);
            }
            if($e->getCode() === 0 && !empty($login)){
                $loginLogService->createLog($adminId, 'admin','login','登录成功');
                $this->success($login);
            }else {
                $loginLogService->createLog($adminId, 'admin','login','登录失败',"【{$e->getCode()}】:".$e->getMessage());
