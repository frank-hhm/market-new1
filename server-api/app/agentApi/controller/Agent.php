...
use app\agentApi\validate\AgentPassValidate;
...
use app\agentApi\services\AgentService;
...
class Agent extends Base
{
    ...
    public function logout(): void
    {
        ...
        $cacheKey = Env::get('redis.jwt').':agent:'.md5($token);
    ...
    public function getAgentSelectByPid(){
        $agent = $this->service->getAgentSelectByIds($this->getAgentChildIds());
    ...
    public function updatePass(): void
    {
        ...
        validate(AgentPassValidate::class)->check($data);
    ...
    public function updateAvatar(){
        ...
        if ($this->service->update($this->agentId, ['avatar' => $fileUrl])) {
            $this->success('修改成功');
        }
        $this->error('失败成功');
    }
    ...
    public function getLevelData(){
        ...
        $levels = array_filter($levels,function ($item){
            return $item['value'] > $this->agentInfo['level']['value'];
        });
    ...
    public function getAgentSelect(){
        ...
        $agent = $this->service->getLevelAgentSelect2($level,$this->getAgentChildIds());
        $this->success('获取成功',$agent);
    }
    /**
     * 获取代理手续费设置
