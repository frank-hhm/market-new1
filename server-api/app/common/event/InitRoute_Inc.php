...
class InitRoute extends Service
{
    ...
    public function handle()
    {
        ...
        // 是否插件
        if (!empty($pathinfoArray[0]) && !in_array($pathinfoArray[0], $GLOBALS['app_array']) ) {
            isset($pathinfoArray[0])  && $this->addon = $pathinfoArray[0];
            !empty($pathinfoArray[1]) && $this->appName = $pathinfoArray[1];
            isset($pathinfoArray[2]) && $this->controller = $pathinfoArray[2];
            isset($pathinfoArray[3]) && $this->action = $pathinfoArray[3];
            Request::addon($this->appName);
            $this->modulePath = "addon/" . $this->addon . '/'. $this->appName;
            $this->setAppPath("addon/" . $this->addon);
        }else{
        ...
        if($this->appName == 'tamsisps'){
            return view(app()->getRootPath() . 'public/tamsisps/index.html');
        }elseif($this->appName == 'web'){
            return view(app()->getRootPath() . 'public/web/index.html');
        }elseif($this->appName == 'h5'){
            return view(app()->getRootPath() . 'public/h5/index.html');
        }elseif($this->appName == 'h5uat'){
            return view(app()->getRootPath() . 'public/h5uat/index.html');
        }

//        $this->routePath = $this->appName .'/' . $this->controller . '/' . $this->action;

        $this->routePath = '/' . $this->controller . '/' . $this->action;
