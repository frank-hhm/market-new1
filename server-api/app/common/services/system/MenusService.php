...
class MenusService extends \app\common\services\BaseService
{
    ...
    public function getList(array $params,$field = '*'): array
    {
        ...
        if(!empty($params['module']) && $params['module'] != 'all'){
     ...
     * 获取后台权限菜单和权限
     */
    public function getMenusList($roleId, int $level,int $type = 1,$module = 0): array
    {
        $rulesStr = $this->getRoleStr($roleId);
        $ruleMap = ['status' => 1];
        $level > 0 && $ruleMap['id'] = $rulesStr;
        $module !== 0 && $ruleMap['module'] = $module;
    ...
    public function getAgentMenusList(int $type = 1): array
    {
        $ruleMap = ['status' => 1,'module' => 2];
        $menusList = $this->dao->getMenusRole(array_merge($ruleMap,['type' => $type]));
        $menusList && $menusList = $menusList->toArray();
        $menusList = ArrayHelper::arr2tree($menusList,'id','pid','children');
        return ['menusList'=>$menusList,'action'=> -1,'homePath'=>$this->getHome($menusList)['menu_path'] ?? ''];
    }
