...
class JwtAuthService extends BaseService
{
    ...
    public function createToken(int $id, string $type, array $params = []): array
    {
        ...
        $res = app(CacheService::class)->setRedisName(CacheKeyConstant::JWT_REDIS_DRIVER)->set(Env::get('redis.jwt').':'.$type.':'.md5($tokenInfo['token']), ['uid' => $id, 'type' => $type, 'token' => $tokenInfo['token'], 'exp' => $exp], (int)$exp, $type);
        if (!$res) {
