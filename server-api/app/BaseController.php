...
abstract class BaseController
{
    ...
    {
        if(empty($phone)) throw new CommonException('手机号错误');
        $cacheService = app(CacheService::class)->setRedisName(CacheKeyConstant::MEMBER_REDIS_DRIVER);
        $res = !empty($code) && $cacheService->get('verify_code:phone:'.$phone) === $code;
        if(!$res) throw new CommonException('动态验证码错误');
        $cacheService->delete('verify_code:phone:'.$phone);
        return true;
    }
}
