    ...
    <template v-slot:content="{ height }">
      ...
      >
        <template #columns>
          ...
          <a-table-column title="类型" data-index="sys_ping" :width="120">
            <template #cell="{ record }">
              <a-tooltip
                :content="record.sell_type.desc || record.sell_type.name"
              >
                <text class="ml5">{{ record.sell_type.name }}</text>
              </a-tooltip>
            </template>
          </a-table-column>
          ...
          <a-table-column title="盈亏" data-index="ploss" :width="120">
            <template #cell="{ record }">
...
import { getOrderPinListApi } from "@/api/order/order";
