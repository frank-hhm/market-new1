...
<script>
	...
	export default {
		...
		onReady() {
			let t = this;
			this.mainData = JSON.parse(JSON.stringify(candlestickOption(this.decimal)));
			timer = setInterval(() => {
		...
		methods: {
			...
			updateTimeData(newValue = 0) {
				...
				if (mainEcharts !== undefined) {
					...
					const option = mainEcharts.getOption();
					// 确保数据存在
					if (option !== undefined && option.series && option.series[0].data) {
						// 获取第一个系列的数据数组
						...
						if (currentActive == 5) {
							...
							if (seconds === 0 && isPush === false) {
								...
								console.log(seriesData[lastIndex])
							}
						} else if (
