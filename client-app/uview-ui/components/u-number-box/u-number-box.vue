<template>
	<view class="u-numberbox">
		<view class="u-icon-minus" @touchstart.stop.prevent="btnTouchStart('minus')" @touchend.stop.prevent="clearTimer"
			:class="{ 'u-icon-disabled': disabled || inputVal <= min ||  (inputVal ===0 &&stepDefaultValue !== false && stepDefaultValue <= min) }" :style="{
				background: bgColor,
		...
		<view class="u-icon-plus" @touchstart.stop.prevent="btnTouchStart('plus')" @touchend.stop.prevent="clearTimer"
			:class="{ 'u-icon-disabled': disabled || inputVal >= max || (inputVal ===0 && stepDefaultValue !== false && stepDefaultValue >= max) }" :style="{
				background: bgColor,
				height: inputHeight + 'rpx',
				color: color,
			}">
	...
	export default {
		...
		methods: {
			...
			computeVal(type) {
				...
				let value = 0;
				// 减
				let _value = this.inputVal
				if (_value === 0 && this.stepDefaultValue !== false) {
					_value = this.stepDefaultValue
				}
				// console.log(_value,this.stepDefaultValue)
				if (type === 'minus') {
					value = this.calcMinus(_value, this.step);
				} else if (type === 'plus') {
					value = this.calcPlus(_value, this.step);
				}
				// 判断是否小于最小值和大于最大值
