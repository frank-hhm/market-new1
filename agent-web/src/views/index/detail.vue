            ...
            <div class="detail-main">
                ...
                <div class="detail-right">
                    <div class="detail-item">
                        ...
                        <div class="desc">{{ agentDetail.account }}</div>
                    </div>
                    <div class="detail-item">
                        <div class="title">姓名：</div>
                        <div class="desc">{{ agentDetail.real_name }}</div>
                    </div>
                    <div class="detail-item" v-if=" agentDetail.last_ip?.value">
                        <div class="title">登录IP：</div>
                        <div class="content">
                            <div class="fz12">{{ agentDetail.last_ip?.value }}</div>
                            <div class="text-grey" v-if="agentDetail.last_ip?.text">{{ agentDetail.last_ip?.text
                                }}</div>
                        </div>
                    </div>
                </div>
...
import adminPassModal from "@/components/agent/update-password.vue";
import { uploadAvatarApi } from "@/api/agent";
import { Result, ResultError } from "@/types";


const { agentDetail } = storeToRefs(useAgentStore());
