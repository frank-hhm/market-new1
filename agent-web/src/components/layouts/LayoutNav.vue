<template>
  <div class="layout-nav" :class="isMobile ? 'mobile' : ''">
    ...
    <div class="layout-nav-right" ref="rightRef">
      ...
      <a-dropdown v-if="agentDetail?.id">
        <div class="layout-nav-user">
          <a-avatar :size="24" :imageUrl="agentDetail?.avatar"></a-avatar>
          <div class="ml10" v-if="!isMobile">
            {{ agentDetail.real_name }}
            ({{ agentDetail.account }})
          </div>
        </div>
...
const websocketMessageCallback = ({ detail }: any) => {
