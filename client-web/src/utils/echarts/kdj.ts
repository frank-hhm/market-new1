const calc_KDJ = (items: any,decimal:number = 2) => {
    //[开盘值, 收盘值, 最低值, 最高值]
    let data = items.map((item: any) => [item[1], item[4], item[3], item[2]])
    let kdjList = exec_KDJ(data,decimal)
    return kdjList
}


//　RSV=（今日收盘价一最近9天的最低价）÷（最近9天的最高价一最近9天的最低价）×100
const exec_KDJ = (value: any,decimal:number) => {
    let rsvList = []
    let kList = []
    let dList = []
    let jList = []
    for (let i = 0; i < value.length; i++) {
        if (i < 8) {
            ...
            let lastK: any = kList[i - 1] == "-" ? 50 : kList[i - 1]
            let lastD: any = dList[i - 1] == "-" ? 50 : dList[i - 1]
            let k: any = (2 / 3 * lastK) + (1 / 3 * rsv)
            let d: any = (2 / 3 * lastD) + (1 / 3 * k)
    ...
    //最近9天的最低价
    function calc_lowest(value: any, i: any) {
        let lowestPrice = 0
        for (let j = i - 8; j <= i; j++) {
            if (lowestPrice == 0) {
    ...
    //最近9天的最高价
    function calc_highest(value: any, i: any) {
        let highestPrice = 0
        for (let j = i - 8; j <= i; j++) {
            if (highestPrice == 0) {
