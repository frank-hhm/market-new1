...
// 故威廉指标WR=Q/R*100
const calc_WR = (value: any, dayCount: number) => {
    let wrList = []
    for (let i = 0; i < value.length; i++) {
        if (i < dayCount - 1 || !value[i][0] || !value[i][1] || !value[i][2] || !value[i][3]) {
            ...
            //[开盘值, 收盘值, 最低值, 最高值]
            let maxValue = Math.max(...list.map((item: any) => parseFloat(item[3])))
            let minValue = Math.min(...list.map((item: any) => parseFloat(item[2])))
