...
export const numberFormat = (value: number | string, degree: number = 2,isValue:boolean = true) => {
    var param: any = {}, point = 'undefined' === typeof degree ? 2 : degree;
    var k = 10000,k1 = 10000000000000, sizes = ['', '万', '亿', '万亿'], i;
    if (Number(value) < k) {
        param.value = Number(value).toFixed(point);
        param.unit = ''
    } else if(Number(value) > k1){
        param.value= (Number(value) / k1).toFixed(point);;
        param.unit = sizes[3];
    } else {
        i = Math.floor(Math.log(Number(value)) / Math.log(k));
        param.value = ((Number(value) / Math.pow(k, i))).toFixed(point);
        param.unit = sizes[i];
    }
    if(isValue){
