<template>
    <a-modal v-model:visible="visible" title="修改密码" @BeforeOk="onSave" @BeforeCancel="close"
        :width="isMobile ? 'calc(100% - 20px)' : '400px'" :top="useSetting().ModalTop" :align-center="false"
        title-align="start" render-to-body>
...
import { updateMemberPasswordApi } from "@/api/member";
...
const onSave = () => {
    proxy?.$refs['createRef'].validate((error: undefined | Record<string, ValidatedError>) => {
        if (error === undefined) {
            ...
            updateMemberPasswordApi(createForm.value)
                .then((res: Result) => {
