        ...
        <a-table :data="orderHold" :loading="isInitMemberStatus" row-key="id" isLeaf :pagination="false" :scroll="{
            ...
            <template #columns>
                ...
                <a-table-column title="盈亏" data-index="buyprice" align="center" :width="120">
                    <template #cell="{ record }">
                        <orderItemYingkuiComponent :order="record"></orderItemYingkuiComponent>
                    </template>
                </a-table-column>
...
import { Notification, Table } from "@arco-design/web-vue";
import orderItemYingkuiComponent from "./order-item-yingkui.vue";
...
const onSell = (item: any) => {
    ...
    orderSellApi({
        ...
        Notification.success({
            title: res.message,
            content: () => {
                let contentString = "产品:" + res.data.product_name + "\n";
                contentString += "盈亏:" + res.data.yingkui + "\n";
                contentString += "平仓价格:" + res.data.sell_price + "\n";
                contentString += "平仓时间:" + res.data.sell_time + "\n";
